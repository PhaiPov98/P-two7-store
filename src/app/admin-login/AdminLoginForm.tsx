...
import Telegram2FAScreen from '@/components/auth/Telegram2FAScreen';
...
export default function AdminLoginForm() {
  ...
  const handle2FASuccess = async (user: any) => {
    login(user);
    await refreshUser();
    success('ចូលផ្ទាំង Admin ជោគជ័យ!', `សូមស្វាគមន៍ Admin ${user.name}`);
    router.push('/admin');
    router.refresh();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if 2FA Telegram approval is required
      if (data.requires2FA) {
      ...
      {twoFARequestId ? (
        <div className="w-full relative z-10">
          <Telegram2FAScreen
            ...
            onCancel={() => setTwoFARequestId(null)}
          />
        </div>
      ) : (
